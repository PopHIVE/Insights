...
# Identify top 5 states for each year
top5_states_all_years <- deaths_state %>%
  group_by(year) %>%
  arrange(desc(rate_deaths_overdose)) %>%
  slice_head(n = 5) %>%
  ungroup() %>%
...
# Filter for those states across all years
top5_trends <- deaths_state %>%
  semi_join(top5_states_all_years, by = c("year", "geography"))

write_xlsx(top5_trends, "./data/top5_horserace_graph.xlsx") #for tableau
```

<iframe src="https://public.tableau.com/views/Book1_17637377516820/Sheet3?:showVizHome=no&amp;:embed=true" width="100%" height="600" frameborder="0">
