...
<iframe src="https://public.tableau.com/views/Book3_17637546219550/Sheet6?:showVizHome=no&amp;:embed=true" width="100%" height="600" frameborder="0">

</iframe>

```{r}
## # Rank all states for each year
state_rank_all_years <- deaths_state %>%
  group_by(year) %>%
  arrange(desc(rate_deaths_overdose)) %>%
  mutate(rank = row_number()) %>%   # assign rank to every state
  ungroup() %>%
  select(year, geography, rate_deaths_overdose, rank)

# Save full ranking for Tableau
write_xlsx(state_rank_all_years, "./data/state_rank_all_years.xlsx")

# Rank all states per year
ranked_states <- deaths_state %>%
  group_by(year) %>%
  arrange(desc(rate_deaths_overdose)) %>%
...
write_xlsx(top10_trends, "./data/top10_horserace_graph.xlsx") #for tableau
...
<iframe src="https://public.tableau.com/views/Overdosedeathratesfrom2015to2025/Sheet2?:showVizHome=no&amp;:embed=true" width="800" height="600" style="border:none;">
...
write_xlsx(geo_source_filtered, "./data/geo_source_filtered.xlsx") #for tableau
```

```{r}
library(tidyverse)

#Defining census regions

census_regions <- list( Northeast = c("Connecticut","Maine","Massachusetts","New Hampshire","Rhode Island","Vermont", "New Jersey","New York","Pennsylvania"),
...
region_lookup <- enframe(census_regions, name="region", value="state") %>% 
  unnest(cols=state)

#Adding to the dataset
deaths_state_region <- deaths_state %>% 
  left_join(region_lookup,by=c("geography"="state"))

#weighted monthly regional rates
regional_trends <- deaths_state_region %>% 
  group_by(region, time) %>% summarise( region_rate_deaths_overdose = sum(n_deaths_overdose) / sum(n_deaths_overdose / rate_deaths_overdose), total_n_deaths_overdose = sum(n_deaths_overdose), .groups="drop" )

# visual trends

library(ggplot2)
ggplot(regional_trends, aes(x=time, y=region_rate_deaths_overdose, color=region)) +
  geom_line(size=1.2) + 
  labs(title="Regional Trends in Overdose Death Rates", x="Time (Month)", y="Rate of Overdose Deaths") + 
  theme_minimal()

#for datawrapper
write_xlsx(regional_trends, "./data/rate_deaths_region.xlsx")
```
